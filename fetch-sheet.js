      ...
      if (err) reject(err);else {
        resolve(rows.map(function (r) {
          return (
            ...
            _.omit(r, ['_xml', 'app:edited', 'save', 'del', '_links']), function (val, key) {
...
var cleanRows = function cleanRows(rows) {
  return rows.map(function (r) {
    return _.mapValues(r, function (val, key) {
      if (key === 'date') {
      ...
      if (!isNaN(val.replace(/[\W]/g, '')) && val.replace(/[\W]/g, '') !== '' && val !== '') {
        return unescape(encodeURIComponent(Number(val.replace(/[\W]/g, ''))));
      }
